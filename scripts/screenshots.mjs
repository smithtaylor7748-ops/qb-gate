...
/** 拍哪几页。id 要在 `PageId` 里，见 `src/lib/nav.ts`。 */
const SHOTS = [
  ['home', 'home.png'],
  ['purity', 'purity.png'],
  ['iplock', 'iplock.png'],
...
function shoot(browser, page, file) {
  ...
  return new Promise((ok, bad) => {
    ...
    p.on('error', bad);
    p.on('exit', (code) => (code === 0 ? ok() : bad(new Error(`浏览器退出码 ${code}`))));
  });
}
...
if (!(await alive())) {
  ...
  server = spawn('npm', ['run', 'demo'], { cwd: ROOT, stdio: 'ignore', shell: true });
  if (!(await waitUp())) {
    server.kill();
    throw new Error('演示服务器 30 秒内没起来。先手动 npm run demo 看看报什么错。');
  }
} else {
