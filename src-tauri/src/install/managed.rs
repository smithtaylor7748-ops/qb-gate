...
fn write_record(root: &Path, app: App, rec: Record) -> Result<()> {
    let mut all = read_records(root);
...
pub fn probe_dir(dir: &Path) -> Probe {
    ...
    let panel_dir = std::env::current_exe().ok().and_then(|p| p.parent().map(Path::to_path_buf));
...
pub fn parse_codex_release(json: &str, asset: &str) -> Result<Release> {
    ...
    let version = tag.trim_start_matches("rust-v").trim_start_matches('v').to_string();
    let a = v
        .get("assets")
        .and_then(|a| a.as_array())
        .and_then(|list| list.iter().find(|x| x.get("name").and_then(|n| n.as_str()) == Some(asset)))
    ...
    let url = a
        ...
        .ok_or_else(|| {
            GateError::Other("Codex 的下载地址不在 github.com/openai/codex 下，面板不跟着它走".into())
        })?
        .to_string();
    let sha256 = a
        .get("digest")
...
async fn claude_release(channel: &str) -> Result<Release> {
    let c = http()?;
    let version = get_text(&c, &format!("{CLAUDE_RELEASES}/{channel}")).await?.trim().to_string();
    if !looks_like_version(&version) {
        return Err(GateError::Other(
            "官方发布源回的不是版本号（多半是当前网络所在地区访问不了 downloads.claude.ai）。".into(),
        ));
    }
...
async fn download(url: &str, dest: &Path, size_hint: Option<u64>, rep: &Reporter, step: u32) -> Result<String> {
    ...
    loop {
        ...
        if let Some(t) = total.filter(|t| *t > 0) {
            ...
            if pct >= last_pct + 5 {
                last_pct = pct;
                rep.phase(step, format!("下载中 {pct}%（{} / {} MB）", got >> 20, t >> 20));
            }
        }
    }
    file.flush()?;
    if let Some(t) = total {
        if got != t {
            return Err(GateError::Other(format!("下载不完整：收到 {got} 字节，应为 {t} 字节")));
        }
    }
...
pub fn place(app: App, src: &Path, target: &Path, want_sha: &str) -> Result<Option<PathBuf>> {
    let backup = if target.exists() {
        ...
        std::fs::rename(target, &b).map_err(|e| {
            GateError::Other(format!("旧的 {} 改不了名（多半正在运行）：{e}", target.display()))
        })?;
        Some(b)
    } else {
        None
...
#[cfg(windows)]
fn lock_backup(b: &Path, log: &mut Vec<String>) {
    let r = crate::gate::acl::current_user_sid().and_then(|sid| crate::gate::acl::lock(b, &sid));
    if let Err(e) = r {
        log.push(format!("留底 {} 没能加上执行锁：{e}", b.display()));
    }
}

#[cfg(not(windows))]
fn lock_backup(_b: &Path, _log: &mut Vec<String>) {}
...
pub async fn install(app: App, channel: &str, rep: &Reporter) -> Result<InstallOutcome> {
    let mut log = Vec::new();
    let root = root();
    ...
    let file_name = rel.url.rsplit('/').next().unwrap_or("download.bin").to_string();
    ...
    let signer = crate::killswitch::signer_of(&bin).await;
    ...
    let bin_sha = sha256_file(&bin).ok_or_else(|| GateError::Other("算不出下载文件的 SHA-256".into()))?;
    ...
    for h in &helpers {
        let name = h.file_name().map(|n| n.to_os_string()).unwrap_or_default();
        let signer = crate::killswitch::signer_of(h).await;
        if crate::install::winget::signature_matches(signer.as_deref(), app.signer()) != Some(true) {
            log.push(format!(
                "辅助程序 {} 的签名没核对通过（{}），没装",
                h.display(),
                signer.unwrap_or_else(|| "读不出".into())
            ));
            continue;
        }
        let dest = dir.join(&name);
        let _ = std::fs::remove_file(&dest);
        match std::fs::rename(h, &dest) {
            Ok(()) => log.push(format!("辅助程序 {} 已就位", dest.display())),
            Err(e) => log.push(format!("辅助程序 {} 放不进去（多半正在运行）：{e}", dest.display())),
        }
    }
    let _ = std::fs::remove_dir_all(&dl);
    write_record(
...
fn copy_verified(src: &Path, dst: &Path) -> Result<()> {
    ...
    for e in std::fs::read_dir(src)? {
        ...
        if from.is_dir() {
            ...
            if !same {
                return Err(GateError::Other(format!("复制 {} 之后内容对不上", from.display())));
            }
        }
    }
    Ok(())
...
pub fn migrate_files(old: &Path, new: &Path) -> Result<Vec<String>> {
    ...
    let apps: Vec<App> = App::ALL.into_iter().filter(|a| app_dir(old, *a).exists()).collect();
    for app in &apps {
        let dst = app_dir(new, *app);
        if dst.exists() && std::fs::read_dir(&dst).map(|mut r| r.next().is_some()).unwrap_or(false) {
            return Err(GateError::Other(format!(
    ...
    for app in &apps {
        ...
        if let Err(e) = move_tree(&src, &dst) {
            ...
            return Err(GateError::Other(if stuck.is_empty() {
                format!("{} 没搬成，已经全部恢复原样，托管目录没有改：{e}", app.label())
            } else {
                format!(
...
pub fn lock_strays(new: &Path) -> usize {
    ...
    let Ok(sid) = crate::gate::acl::current_user_sid() else {
        return 0;
    };
    ...
    files.iter().filter(|f| crate::gate::acl::lock(f, &sid).is_ok()).count()
}
...
pub fn plan_claude_externals(
    ...
    for i in installs {
        ...
        match i.kind {
            ...
            Kind::Native | Kind::NativeVersion | Kind::Programs | Kind::Path => out.push(External {
                app: App::ClaudeCode,
    ...
    for s in stale {
        ...
        if !is_inside(s, managed_root) && !crate::install::inventory::is_desktop_managed_path(s, roaming) {
            out.push(External {
                app: App::ClaudeCode,
                method: Method::Delete,
    ...
    if winget {
        out.push(External {
            ...
            action: "winget uninstall --id Anthropic.ClaudeCode（连 WinGet\\Links 的 shim 与登记一起）".into(),
        });
    }
    if scoop {
        out.push(External {
...
pub fn plan_codex_externals(found: &[PathBuf], managed_root: &Path) -> Vec<External> {
    ...
    if winget {
        out.push(External {
            ...
            method: Method::Winget,
            target: "OpenAI.Codex".into(),
            action: "winget uninstall --id OpenAI.Codex（连 WinGet\\Links 的 shim 与登记一起）".into(),
        });
    }
    out
}
...
async fn stale_registrations(app: App, prefix: &str) -> Vec<External> {
    ...
    rows.into_iter()
        .filter_map(|r| {
            ...
            let loc = r.get("Location").and_then(|l| l.as_str()).unwrap_or("").trim().trim_matches('"').to_string();
...
fn prune_empty_parents(p: &Path, stop_at: &[PathBuf]) {
    ...
    while let Some(d) = cur {
        if stop_at.iter().any(|s| crate::install::inventory::same_path(s, &d)) {
            break;
        }
...
pub async fn cleanup(app: App) -> Result<CleanupReport> {
    ...
    let plan: Vec<External> = externals().await.into_iter().filter(|e| e.app == app).collect();
    ...
    for e in &plan {
        let r: std::result::Result<String, String> = match e.method {
            Method::Npm => {
                ...
                for d in npm_staging_dirs(&roaming.join("npm").join("node_modules").join(scope), name) {
                    match std::fs::remove_dir_all(&d) {
                        Ok(()) => rep.done.push(format!("删掉 npm 中断留下的临时目录 {}", d.display())),
            ...
            Method::Winget => run(
                "winget",
                &[
                    "uninstall", "--id", &e.target, "-e", "--silent",
            ...
            Method::Registry => run(
                ...
                &[
                    "delete",
                    &format!("HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{}", e.target),
                    "/f",
                ],
            )
            .await
            ...
            Method::Delete => {
                ...
                let signer = crate::killswitch::signer_of(&p).await;
                match crate::install::winget::signature_matches(signer.as_deref(), app.signer()) {
    ...
    if app == App::ClaudeCode {
        rep.notes.push(
            "~\\.local\\bin 目录本身与 PATH 里的这一项没动 —— uv、pipx 等别的工具也往那里装东西。".into(),
        );
    }
    Ok(rep)
}
...
mod tests {
    ...
    fn claude_manifest_gives_the_official_url_checksum_and_size() {
        ...
        assert_eq!(r.url, "https://downloads.claude.ai/claude-code-releases/2.1.268/win32-x64/claude.exe");
        assert_eq!(r.sha256.as_deref(), Some("1e472bcfd49449e73ef76698c87f6583055aeb88dc41ca190a9c12c6791b5eea"));
    ...
    fn codex_release_is_pinned_to_the_openai_repo() {
        ...
        assert_eq!(r.sha256.as_deref(), Some("53685f9f6bd171d4bd7d6c2be724fc04f6737a4001eb8471ec59824e5adc8042"));
    ...
    fn placing_keeps_a_backup_and_verifies_the_hash() {
        ...
        let backup = place(App::ClaudeCode, &src, &target, &sha).unwrap().expect("有旧的就要留底");
        assert_eq!(std::fs::read(&target).unwrap(), b"new");
        assert_eq!(std::fs::read(&backup).unwrap(), b"old");
        // 留底名必须落进 stale_copies 的清理范围。
        assert!(backup.file_name().unwrap().to_string_lossy().starts_with("claude.exe.old."));
        ...
        assert_eq!(std::fs::read(&target).unwrap(), b"new", "失败时原来那份要回到原位");
    }
    ...
    fn migrating_moves_everything_and_refuses_to_overwrite() {
        ...
        write_record(&old, App::Codex, Record { version: "0.154.0".into(), ..Default::default() }).unwrap();
        ...
        assert_eq!(record_of(&new, App::Codex).unwrap().version, "0.154.0", "记录要跟着搬");
        ...
        assert_eq!(std::fs::read(exe_in(&other, App::ClaudeCode)).unwrap(), b"theirs");
    }
    ...
    fn a_failed_migration_puts_everything_back() {
        ...
        assert_eq!(std::fs::read(&helper).unwrap(), b"runner", "删到一半的要补回来");
        assert_eq!(record_of(&old, App::Codex).unwrap().version, "0.154.0");
        assert!(!app_dir(&new, App::ClaudeCode).exists(), "新目录里不能留一份没人锁的 claude.exe");
    ...
    fn a_local_ntfs_dir_passes_the_lock_probe() {
        ...
        assert!(std::fs::read_dir(t.0.join("apps")).unwrap().next().is_none(), "探针文件要删掉");
    ...
    fn claude_externals_leave_other_programs_and_the_managed_copy_alone() {
        ...
        let mk = |kind, p: &str| Install { kind, path: PathBuf::from(p), lockable: true, launchable: true, preferred: false };
        let managed = Path::new(r"C:\M");
        let installs = vec![
            mk(Kind::Managed, r"C:\M\claude-code\claude.exe"),
            mk(Kind::Native, r"C:\Users\me\.local\bin\claude.exe"),
            mk(Kind::NativeVersion, r"C:\Users\me\.local\share\claude\versions\2.1.267"),
            mk(Kind::NativeVersion, r"C:\Users\me\.claude\downloads\claude-2.1.263-win32-x64.exe"),
            mk(Kind::Npm, r"C:\Users\me\AppData\Roaming\npm\claude.cmd"),
            mk(Kind::NpmNative, r"C:\Users\me\AppData\Roaming\npm\node_modules\@anthropic-ai\x\claude.exe"),
            mk(Kind::Winget, r"C:\Users\me\AppData\Local\Microsoft\WinGet\Links\claude.exe"),
            mk(Kind::DesktopManaged, r"C:\Users\me\AppData\Roaming\Claude\claude-code\2.1.266\claude.exe"),
            mk(Kind::Editor, r"C:\Users\me\.vscode\extensions\anthropic.claude-code-1\claude.exe"),
            mk(Kind::DesktopStub, r"C:\Users\me\AppData\Local\AnthropicClaude\claude.exe"),
        ];
        let plan = plan_claude_externals(&installs, &[], managed, Some(Path::new(r"C:\Users\me\AppData\Roaming")));
        let deletes: Vec<&str> = plan.iter().filter(|e| e.method == Method::Delete).map(|e| e.target.as_str()).collect();
        assert_eq!(deletes.len(), 3, "{deletes:?}");
        assert!(deletes.iter().all(|d| !d.contains("AnthropicClaude") && !d.contains(".vscode") && !d.contains(r"Roaming\Claude\")));
        assert!(!deletes.iter().any(|d| d.starts_with(r"C:\M")), "托管那份绝不在清理表里");
        // npm 两种合成一次卸载，winget 一次卸载。
        assert_eq!(plan.iter().filter(|e| e.method == Method::Npm).count(), 1);
        assert_eq!(plan.iter().filter(|e| e.method == Method::Winget).count(), 1);
        // 账户与配置根本不会出现。
        assert!(plan.iter().all(|e| !e.target.contains("credentials") && !e.target.contains("claude-profile")));
    }
    ...
    fn codex_externals_skip_the_desktop_app_and_msix_copies() {
        ...
        assert_eq!(targets, vec![r"C:\Users\me\.local\bin\codex.exe", "@openai/codex", "OpenAI.Codex"]);
    }
