...
"#;
    let out = crate::process::hidden_tokio(tokio::process::Command::new("powershell"))
        ...
        .await
        .ok()?;
    let v: serde_json::Value = serde_json::from_str(String::from_utf8_lossy(&out.stdout).trim()).ok()?;
    let first = v.as_array()?.first()?;
    Some(MsixPackage {
        version: first.get("Version").and_then(|x| x.as_str()).map(String::from),
        location: first.get("InstallLocation").and_then(|x| x.as_str()).map(String::from),
    })
}
...
pub fn codex_candidates() -> Vec<PathBuf> {
    ...
    if let Ok(rd) = std::fs::read_dir(wg.join("Packages")) {
        for e in rd.filter_map(|e| e.ok()) {
            if e.file_name().to_string_lossy().to_lowercase().starts_with("openai.codex") {
                if let Ok(files) = std::fs::read_dir(e.path()) {
                    ...
                    out.extend(files.filter_map(|f| f.ok()).map(|f| f.path()).filter(|p| {
                        ...
                        n == "codex.exe" || (n.starts_with("codex-") && n.ends_with("-pc-windows-msvc.exe"))
                    }));
                }
            }
        }
...
pub async fn codex() -> Software {
    ...
    let is_managed = found
        .as_deref()
        .is_some_and(|p| crate::install::inventory::same_path(p, &managed::exe(managed::App::Codex)));
    ...
    let version = match &found {
        Some(_) if is_managed => managed::record_of(&managed::root(), managed::App::Codex).map(|r| r.version),
...
mod tests {
    ...
    fn the_managed_copy_comes_first_then_npm_global() {
        ...
        assert_eq!(c[1].file_name().and_then(|n| n.to_str()), Some("codex.cmd"), "其次是 npm 全局那份");
