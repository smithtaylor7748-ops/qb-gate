...
pub fn chrome_candidates() -> Vec<PathBuf> {
    ...
    vec![
        ...
        pf86.join("Google").join("Chrome").join("Application").join("chrome.exe"),
...
async fn process_count(name: &str) -> Option<usize> {
    ...
    serde_json::from_str::<Vec<String>>(&text).ok().map(|v| v.len())
}
...
pub async fn claude_traces() -> TraceReport {
    ...
    note(
        "install",
        "Claude Code 版本库",
        home().join(".local").join("share").join("claude").join("versions"),
        "每个版本一份完整二进制",
    );
...
pub async fn reinstall(rep: &Reporter) -> Result<String> {
...
mod tests {
    ...
    fn user_data_is_the_profile_root_not_the_program_dir() {
        ...
        assert!(
            !p.display().to_string().to_lowercase().contains("application"),
            "指到安装目录去了：{}",
            p.display()
        );
    }
