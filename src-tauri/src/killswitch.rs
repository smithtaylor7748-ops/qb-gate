...
pub fn triage(p: &RawProcess, data_dir: &str, self_chain: &[u32]) -> Triage {
    ...
    match classify(p, data_dir) {
        ...
        None => Triage::Spare(format!("PID {} {} —— 两条证据都不满足，放过", p.pid, p.name)),
    }
}
...
async fn enumerate() -> Result<Snapshot> {
    ...
    if text.is_empty() {
        ...
        return Err(GateError::Other(format!(
            ...
            if err.is_empty() { "stderr 也是空的" } else { err }
        )));
    }
...
pub fn parse_desktop_rows(text: &str, local: Option<&std::path::Path>) -> Result<Vec<(u32, String)>> {
    let text = text.trim();
...
pub fn desktop_processes() -> Result<Vec<(u32, String)>> {
    ...
    if stdout.trim().is_empty() {
        ...
        return Err(GateError::Other(format!(
            ...
            out.status.code().unwrap_or(-1),
            if err.is_empty() { "stderr 也是空的".into() } else { err }
        )));
    }
...
pub async fn execute() -> Result<KillReport> {
    ...
    for t in &report.targets {
        ...
        match out {
            ...
            Ok(o) => {
                // 常见且无害的一种失败：这个 PID 刚才被别人的 /T 当子进程
                // 一起收走了。报成失败会让用户以为没关干净。
                let err = String::from_utf8_lossy(&o.stderr).trim().to_string();
                let gone = err.contains("找不到") || err.to_lowercase().contains("not found");
                if gone {
                    report.killed.push(t.pid);
                } else {
                    report.failed.push((t.pid, err));
                }
            }
...
mod tests {
    ...
    fn snapshot_parses_single_element_output() {
        ...
        let raw = r#"{"SelfChain":[10],"Procs":[{"ProcessId":1,"ParentId":10,"Name":"claude.exe"}]}"#;
    ...
    fn npm_claude_code_is_recognised_by_runtime_plus_package_path() {
        ...
        // 正斜杠写法也要认。
        let p = proc("node.exe", Some("node C:/x/node_modules/@anthropic-ai/claude-code/cli.js"), None);
        assert_eq!(classify(&p, DD), Some(Evidence::NpmPackage));
    }
    ...
    fn node_alone_or_package_path_alone_is_not_enough() {
        ...
        let p = proc(
            "code.exe",
            Some(r"code.exe C:\Users\me\AppData\Roaming\npm\node_modules\@anthropic-ai\claude-code\README.md"),
            None,
        );
        assert_eq!(classify(&p, DD), None);
        // 名字相近的别的包不算。
        let p = proc("node.exe", Some(r"node C:\x\node_modules\@anthropic-ai\sdk\index.js"), None);
        assert_eq!(classify(&p, DD), None);
    }
    ...
    fn roles_tell_desktop_from_code() {
        ...
        let role = |path: &str| role_of(&at(path), Evidence::AnthropicSigned, Some(local), Some(roaming));

        assert_eq!(role(r"C:\Users\me\AppData\Local\AnthropicClaude\app-1.49585.0\claude.exe"), Role::Desktop);
        // 桌面端 Code 页拉起的会话归桌面端。
        assert_eq!(role(r"C:\Users\me\AppData\Roaming\Claude\claude-code\2.1.266\claude.exe"), Role::Desktop);
        assert_eq!(role(r"C:\Users\me\.local\bin\claude.exe"), Role::Code);

        let bridge = proc("python.exe", Some("x"), None);
        assert_eq!(role_of(&bridge, Evidence::BridgeAndDataDir, Some(local), Some(roaming)), Role::Bridge);
        assert_eq!(role_of(&bridge, Evidence::NpmPackage, Some(local), Some(roaming)), Role::Code);
    }
    ...
    fn desktop_rows_are_filtered_in_rust_not_in_the_script() {
        let local = std::path::Path::new(r"C:\Users\O'Brien\AppData\Local");
        let raw = r#"[
            {"ProcessId":10,"ExecutablePath":"C:\\Users\\O'Brien\\AppData\\Local\\AnthropicClaude\\app-1.2.3\\claude.exe"},
            {"ProcessId":11,"ExecutablePath":"C:\\Users\\O'Brien\\.local\\bin\\claude.exe"},
            {"ProcessId":12,"ExecutablePath":null},
            {"ProcessId":13,"ExecutablePath":"C:\\Users\\O'Brien\\AppData\\Local\\AnthropicClaude\\Update.exe"}
        ]"#;
        let got = parse_desktop_rows(raw, Some(local)).unwrap();
        assert_eq!(got.iter().map(|(p, _)| *p).collect::<Vec<_>>(), vec![10, 13]);
    ...
    fn desktop_rows_distinguish_none_from_error() {
        ...
        assert!(parse_desktop_rows("", None).is_err(), "空输出只可能是出错了");
    ...
    fn desktop_script_uses_the_51_safe_json_form() {
        let call = DESKTOP_SCRIPT
            ...
            .find(|l| l.trim_start().starts_with("ConvertTo-Json"))
            .unwrap();
        assert!(call.contains("-InputObject") && !call.contains("-AsArray"), "{call}");
