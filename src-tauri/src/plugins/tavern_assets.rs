...
pub const CATEGORIES: &[Category] = &[
    Category { id: "worlds", label: "世界书", dir: "worlds" },
    Category { id: "characters", label: "角色卡", dir: "characters" },
    Category { id: "presets", label: "预设", dir: "OpenAI Settings" },
    Category { id: "groups", label: "群组", dir: "groups" },
    Category { id: "chats", label: "聊天记录", dir: "chats" },
    Category { id: "extensions", label: "扩展", dir: "extensions" },
];
...
fn backups_root() -> PathBuf {
    crate::gate::state_dir().join("tavern-backups")
}
...
pub fn backup() -> Result<BackupEntry> {
    ...
    crate::gate::log::write(&format!("酒馆资产已备份到 {}", dest.display()));
    Ok(BackupEntry {
        size: dir_size(&dest),
...
pub fn restore(backup_id: &str) -> Result<String> {
    ...
    crate::gate::log::write(&format!("酒馆资产已从 {backup_id} 恢复"));
    Ok(format!(
