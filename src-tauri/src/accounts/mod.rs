...
impl AccountRoots {
    pub fn current() -> Self {
        let bridge = crate::plugins::sillytavern::bridge_data_dir();
        Self {
            panel: crate::gate::state_dir(),
    ...
    fn desktop_dir(&self, label: &str) -> Option<PathBuf> {
        self.appdata.as_ref().map(|a| a.join(format!("Claude-{label}")))
    }
}
...
pub fn desktop_state(r: &AccountRoots) -> DesktopState {
    let Some(link) = r.desktop_link() else {
        return DesktopState { managed: false, active: None };
    };
    match junction_target(&link) {
        Some(t) => {
            let name = t.file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_default();
            // 文件系统不分大小写，前缀也别分。
            let active = name
                .get(..7)
                .filter(|p| p.eq_ignore_ascii_case("Claude-"))
                .map(|_| name[7..].to_string());
            DesktopState { managed: true, active }
        }
        None => DesktopState { managed: false, active: None },
    }
}
...
pub fn create_slot(r: &AccountRoots, label: &str) -> Result<CreateOutcome> {
    ...
    if activated {
        repoint(&r.link(), &dir, &mut notes)?;
        if let Some(b) = &r.bridge {
            repoint(&b.join(LINK), &dir, &mut notes)?;
...
pub fn switch_in(r: &AccountRoots, label: &str, mode: DesktopMode) -> Result<SwitchOutcome> {
    ...
    let from = active_label(r);
    let desktop = plan_desktop(r, label, mode, from.as_deref());

    let mut out = SwitchOutcome::default();
    let prev_cli = junction_target(&r.link());
    let bridge_link = r.bridge.as_ref().map(|b| b.join(LINK));
    let prev_bridge = bridge_link.as_deref().and_then(junction_target);

    // 失败时把已经换过的指向还原。尽力而为 —— 还原本身失败也要让人知道。
    let rollback = |out: &mut SwitchOutcome, cli: bool, bridge: bool| {
        let mut sink = Vec::new();
        if cli {
            if let Some(p) = &prev_cli {
                if repoint(&r.link(), p, &mut sink).is_err() {
                    out.notes.push("还原 Claude Code 的指向也失败了，请到账户页手动再切一次".into());
                }
            }
        }
        if bridge {
            if let (Some(l), Some(p)) = (&bridge_link, &prev_bridge) {
                if repoint(l, p, &mut sink).is_err() {
                    out.notes.push("还原酒馆桥接的指向也失败了".into());
                }
            }
        }
    };
    ...
    // ---- 1. Claude Code
    repoint(&r.link(), &target, &mut out.notes)?;
    out.switched.push(format!("Claude Code → {label}"));

    // ---- 2. 酒馆桥接：跟面板指向同一个具体目录
    if let (Some(b), Some(l)) = (&r.bridge, &bridge_link) {
        if let Err(e) = repoint(l, &target, &mut out.notes) {
            rollback(&mut out, true, false);
            return Err(GateError::Other(format!("酒馆桥接的指向换不过去：{e}")));
        }
        // 旧的账户管理脚本读这个文件，别让它看到的跟实际对不上。
        let _ = std::fs::write(b.join("active-profile.txt"), label);
    ...
    if let Some(d) = desktop {
        ...
        let step = (|| -> Result<()> {
            if let Some(to) = &d.adopt_to {
                std::fs::rename(&d.link, to)?;
                adopted = true;
            }
            if d.create {
                std::fs::create_dir_all(&d.target)?;
            }
            repoint(&d.link, &d.target, &mut out.notes)
        })();
        if let Err(e) = step {
            // 真目录已经改了名、后面却失败了：把它挪回原处。不挪的话下次打开桌面端
            // 会在 %APPDATA%\Claude 建一份空的，看起来就像被登出了。
            if let (true, Some(to)) = (adopted, &d.adopt_to) {
                if std::fs::symlink_metadata(&d.link).is_err() && std::fs::rename(to, &d.link).is_err() {
                    out.notes.push(format!(
                        "桌面端原来的资料目录现在在 {}，没能挪回 {}，请手动改回去",
                        to.display(),
                        d.link.display()
                    ));
                }
            }
            rollback(&mut out, true, true);
            let extra = if out.notes.is_empty() { String::new() } else { format!("（{}）", out.notes.join("；")) };
            return Err(GateError::Other(format!("桌面端的资料目录换不过去：{e}{extra}")));
        }
        if let Some(to) = &d.adopt_to {
            out.notes.push(format!("桌面端原来的资料目录已存为 {}", to.display()));
        }
        if d.create {
            out.notes.push(format!(
...
pub fn sync_bridge(r: &AccountRoots) -> SyncReport {
    ...
    if let Ok(m) = std::fs::symlink_metadata(&bridge_link) {
        if !is_reparse_point(&m) && m.is_dir() {
            ...
            match std::fs::rename(&bridge_link, b.join(format!("{PREFIX}{label}"))) {
                Ok(()) => rep.done.push(format!("酒馆桥接的单账户目录已认作槽位 {label}")),
                Err(e) => rep.failed.push(format!("酒馆桥接的 claude-profile 改不了名：{e}")),
            }
        }
    }
    ...
    for bpath in entries {
        ...
        if std::fs::symlink_metadata(&ppath).is_err() {
            match std::fs::rename(&bpath, &ppath).and_then(|_| {
                ...
                Ok(()) => rep.done.push(format!("酒馆桥接的槽位 {label} 已搬进面板，原处留了联结点")),
                Err(e) => rep.failed.push(format!("槽位 {label} 搬不过来（多半是酒馆桥接正在用）：{e}")),
            }
            continue;
        }
        ...
        if let (Some(bt), Some(pt)) = (cred_mtime(&bpath), cred_mtime(&ppath)) {
            if bt > pt {
                ...
                if let Err(e) = replace_keeping_backup(&bpath.join(".credentials.json"), &pc, &keep) {
                    rep.failed.push(format!("槽位 {label}：酒馆桥接那份凭证更新，但拷不过来：{e}"));
                    continue;
                }
            ...
            if std::fs::copy(bpath.join(".credentials.json"), ppath.join(".credentials.json")).is_ok() {
                rep.done.push(format!("槽位 {label}：面板这份没登录过，已拷入酒馆桥接那份的凭证"));
            }
        }
        ...
        match std::fs::rename(&bpath, &superseded).and_then(|_| {
            ...
            Err(e) => rep.failed.push(format!("槽位 {label} 合并不了（多半是酒馆桥接正在用）：{e}")),
        }
    }
    ...
    match active_slot_dir(r) {
        Some(active) => {
            ...
            if !was.as_deref().is_some_and(|w| crate::install::inventory::same_path(w, &active)) {
        ...
        None => {
            ...
            if let Some(l) = bridge_active.filter(|l| r.slot_dir(l).is_dir()) {
                ...
                match ok {
                    Ok(()) => rep.done.push(format!("面板接上了酒馆桥接原来的当前槽位 {l}")),
                    Err(e) => rep.failed.push(format!("接不上酒馆桥接原来的当前槽位 {l}：{e}")),
                }
            }
        }
    }
...
mod tests {
    ...
    fn switching_points_code_and_bridge_at_the_same_concrete_dir() {
        ...
        assert_eq!(active_slot_dir(&m.roots).as_deref().map(|d| crate::install::inventory::same_path(d, &main)), Some(true));
        ...
        assert_eq!(std::fs::read_to_string(m.bridge().join("active-profile.txt")).unwrap(), "NEW1");
    ...
    fn switching_desktop_no_longer_checks_whether_it_is_running() {
        ...
        assert!(out.switched.iter().any(|s| s.starts_with("桌面端")), "{:?}", out.switched);
        assert!(points_at(&m.appdata().join("Claude"), &m.appdata().join("Claude-NEW1")));
        assert_eq!(active_label(&m.roots).as_deref(), Some("NEW1"));
    }
    ...
    fn creating_the_first_slot_activates_it_without_touching_the_desktop() {
        ...
        assert!(!is_reparse_point(&std::fs::symlink_metadata(&real).unwrap()));
    ...
    fn duplicated_credentials_are_merged_into_one_copy() {
        ...
        for l in ["main", "NEW1"] {
            // 桥接那边成了指向面板槽位的联结点。
            assert!(points_at(&b.join(format!("{PREFIX}{l}")), &m.roots.slot_dir(l)), "{l}");
        }
        // 桥接的当前跟面板统一。
        assert!(points_at(&b.join(LINK), &m.roots.slot_dir("main")));
        assert!(rep.done.iter().any(|d| d.contains("NEW1") && d.contains("main")), "{:?}", rep.done);
        // 原目录留底，不删。
        ...
        assert!(!rep.done.iter().any(|d| d.contains("替换")), "{:?}", rep.done);
        assert!(!m
            .roots
            .slot_dir("main")
            .read_dir()
        ...
        assert!(again.done.is_empty() && again.failed.is_empty(), "{again:?}");
    }
    ...
    fn the_fresher_credential_wins_the_merge() {
        ...
        let rep = sync_bridge(&m.roots);
        assert!(rep.failed.is_empty(), "{:?}", rep.failed);
        assert_eq!(std::fs::read_to_string(p.join(".credentials.json")).unwrap(), "new");
        // 面板原来那份留底。
        let backups = std::fs::read_dir(&p)
            .unwrap()
            .filter_map(|e| e.ok())
            .filter(|e| e.file_name().to_string_lossy().starts_with(".credentials.json.before-merge-"))
            .count();
        assert_eq!(backups, 1);
    }
