...
pub const PRESET_COMMON: &[&str] = &[
    "US", "CA", "GB", "IE", "DE", "FR", "NL", "SE", "CH", "ES", "IT", "PL", "AU", "NZ", "JP",
    "KR", "SG", "TW",
];
...
mod tests {
    ...
    fn ip_allowed_and_country_allowed_passes() {
        let j = judge(&reading(&[("ippure", "US"), ("cloudflare", "US")]), &allow(), &us());
    ...
    fn country_outside_the_list_is_rejected() {
        ...
        assert_eq!(
            j,
            Judgement::CountryNotAllowed { ip: IP.into(), country: "HK".into() }
        );
    }
    ...
    fn may_add_is_unchanged_when_the_layer_is_off() {
        assert_eq!(may_add(&reading(&[("ippure", "HK")]), &[]), Ok(IP.to_string()));
    }
    ...
    fn presets_exclude_restricted_regions() {
        ...
        for r in ["HK", "MO", "CN"] {
            assert!(!PRESET_COMMON.contains(&r), "{r} 属受限地区，不该进默认预设");
        }
