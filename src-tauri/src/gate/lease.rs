...
pub fn path() -> PathBuf {
    super::state_dir().join("lease.json")
}
...
pub fn persist(l: &Lease) {
    ...
    let r = match serde_json::to_string_pretty(l) {
        Ok(text) => std::fs::write(&p, text).map_err(|e| e.to_string()),
        Err(e) => Err(e.to_string()),
    };
    if let Err(e) = r {
        super::log::write(&format!("租约落盘失败（不影响本次放行）：{e}"));
    }
}
...
pub fn load() -> Option<Lease> {
    ...
    let l: Lease = serde_json::from_str(&text).ok()?;
...
mod tests {
    ...
    fn mutating_a_lease_never_touches_the_disk() {
        ...
        let mut l = Lease::default();
        l.grant_with_mode("test-holder", vec![PathBuf::from("x")], Some(WatchMode::Cli));
        l.release();
    ...
    fn an_older_record_without_a_mode_still_loads() {
        let back: Lease =
            serde_json::from_str(r#"{"holder":"claude-code","granted":[]}"#).unwrap();
        assert!(back.is_held());
