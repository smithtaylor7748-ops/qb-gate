...
/// Repair only the exact NULL-DACL defect produced by historical QB Gate unlocks.
/// Existing explicit permissions (including intentional broad grants) are never reset.
pub fn repair_legacy_null_dacl(path: &Path) -> Result<bool> {
    if read_dacl(path)?.dacl.is_null() {
        restore_inherited(path)?;
        Ok(true)
    } else {
        Ok(false)
    }
...
mod tests {
    ...
    fn a_second_lock_cycle_still_leaves_the_file_readable() {
        ...
        let path = std::env::temp_dir().join(format!("qb-gate-relock-{}.bin", std::process::id()));
        std::fs::write(&path, b"stand-in").expect("写替身文件");
    ...
    fn locking_a_damaged_file_repairs_it_instead_of_keeping_it_unreadable() {
        ...
        let path = std::env::temp_dir().join(format!("qb-gate-damaged-{}.bin", std::process::id()));
        std::fs::write(&path, b"stand-in").expect("写替身文件");
    ...
    fn missing_file_reports_not_found_rather_than_panicking() {
        ...
        assert!(matches!(is_locked(&p, &sid), Err(GateError::NotFound(_))));
    }
}

/// 当前用户对这个文件有没有被 Deny 掉执行权。
///
/// 原来是 `gate::is_locked` —— 一个只做「取当前用户 SID 再问一次 ACL」的
/// 两行包装。但 `install` 的升级与版本库流程都要问这一句，于是整个安装模块
/// 为了它而依赖了门禁。ACL 查询本来就是这一层的事。
pub fn locked(p: &std::path::Path) -> crate::error::Result<bool> {
    is_locked(p, &current_user_sid()?)
}
