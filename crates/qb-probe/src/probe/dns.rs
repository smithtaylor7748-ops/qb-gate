...
fn score_report(resolvers: &[Resolver]) -> (u8, Option<bool>) {
    ...
    let ethernet: Vec<&Resolver> = resolvers
        .iter()
        .filter(|r| {
        ...
        .collect();
    let ethernet_safe = if ethernet.is_empty() {
        None
    } else {
        Some(ethernet.iter().all(|r| !r.is_private))
    };
    let mut score = 0u8;
    if ethernet_safe == Some(true) {
        score += 50;
    }
...
mod tests {
    ...
    fn ethernet_safe_report_gets_full_score() {
        let r = vec![
            res("1.1.1.1", false, false, None),
            res("8.8.8.8", false, true, Some("以太网")),
        ];
