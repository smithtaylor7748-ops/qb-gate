...
#[ts(export, rename = "ClaudeInstall")]
pub struct Install {
    pub kind: Kind,
    pub path: PathBuf,
    pub lockable: bool,
...
fn excluded(p: &Path) -> bool {
    is_desktop_runtime_copy(p) || is_vendored(p) || p.parent().is_some_and(is_electron_dir)
}
...
pub fn is_desktop_process_path(path: &Path, local: Option<&Path>) -> bool {
    ...
    if let Some(l) = local {
        let prefix = format!(
            "{}\\",
            norm(&l.join("AnthropicClaude")).trim_end_matches('\\')
        );
...
pub fn scan(r: &Roots) -> Vec<Install> {
    ...
    if let Some(m) = &r.managed {
        ...
        // Interrupted staging directories can also contain complete runnable copies.
        for entry in entries(&dir) {
            let stage = entry.path();
            if name_lower(&stage).starts_with(".rollback-") && stage.is_dir() {
                for name in ["claude.exe", "claude.exe.before"] {
    ...
    if let Some(ro) = &r.roaming {
        ...
        found.extend(
            find_named(
                &npm.join("node_modules").join("@anthropic-ai"),
                "claude.exe",
                6,
            )
            .into_iter()
...
pub fn stale_copies(r: &Roots) -> Vec<PathBuf> {
    ...
    for d in dirs {
        for e in entries(&d) {
            let p = e.path();
            if p.is_file()
                && name_lower(&p).starts_with("claude.exe.old.")
                && !out.iter().any(|x| same_path(x, &p))
            {
                out.push(p);
            }
        }
...
mod tests {
    ...
    fn a_winget_only_machine_launches_the_copy_it_locks() {
        ...
        assert_eq!(
            preferred_exe(&r).as_deref().map(|q| same_path(q, &exe)),
            Some(true)
        );
    }
    ...
    fn the_managed_copy_wins_over_everything_and_is_locked() {
        ...
        assert_eq!(
            preferred_exe(&r).as_deref().map(|q| same_path(q, &managed)),
            Some(true)
        );
        assert!(lockable_paths(&r)
            .iter()
            .any(|(q, k)| same_path(q, &managed) && *k == Kind::Managed));
    ...
    fn native_versions_are_locked_but_never_launched() {
        ...
        assert_eq!(
            kinds(&all),
            vec![Kind::NativeVersion, Kind::NativeVersion],
            "临时文件不能算"
        );
    ...
    fn desktop_runtime_copies_never_appear() {
        ...
        assert!(
            preferred_cli(&r).is_none(),
            "存根是桌面端，不能当 Claude Code 起"
        );
    }
    ...
    fn a_profile_reached_through_a_junction_is_counted_once() {
        ...
        assert_eq!(
            preferred_cli(&r).map(|i| i.kind),
            Some(Kind::DesktopManaged)
        );
    }
    ...
    fn npm_launches_but_cannot_be_locked() {
        ...
        assert!(lockable_paths(&r)
            .iter()
            .any(|(q, k)| same_path(q, &inner) && *k == Kind::NpmNative));
    ...
    fn editor_extension_binaries_are_locked() {
        ...
        let cursor =
            t.file(r"home\.cursor\extensions\anthropic.claude-code-2.1.260\bin\claude.exe");
        ...
        assert!(locks
            .iter()
            .any(|(q, k)| same_path(q, &exe) && *k == Kind::Editor));
    ...
    fn desktop_process_paths_are_recognised_without_string_splicing() {
        let local = Path::new(r"C:\Users\Demo'Quote [x]\AppData\Local");
        // 用户名带 ' 和 [ ]：旧版拼进 PowerShell 就坏在这里。
        assert!(is_desktop_process_path(
            Path::new(
                r"C:\Users\Demo'Quote [x]\AppData\Local\AnthropicClaude\app-1.2.3\claude.exe"
            ),
            Some(local)
        ));
        assert!(is_desktop_process_path(
            Path::new(r"c:/users/demo'quote [x]/appdata/local/anthropicclaude/Update.exe"),
            Some(local)
        ));
        // MSIX 包目录。
        assert!(is_desktop_process_path(
        ...
        // 执行别名不是包目录。
        assert!(!is_desktop_process_path(
            Path::new(r"C:\Users\Demo'Quote [x]\AppData\Local\Microsoft\WindowsApps\claude.exe"),
            Some(local)
        ));
        // Claude Code 的副本一个都不能认成桌面端 —— 认错了看门狗会连它一起杀。
        for cli in [
            r"C:\Users\Demo'Quote [x]\.local\bin\claude.exe",
            r"C:\Users\Demo'Quote [x]\AppData\Roaming\Claude\claude-code\2.1.260\claude.exe",
            r"C:\Users\Demo'Quote [x]\AppData\Local\AnthropicClaudeX\claude.exe",
        ] {
            assert!(
                !is_desktop_process_path(Path::new(cli), Some(local)),
                "{cli}"
            );
        }
    }
    ...
    fn desktop_managed_paths_cover_every_profile() {
        ...
        assert!(!is_desktop_managed_path(
            Path::new(r"C:\Users\me\.local\bin\claude.exe"),
            Some(ro)
        ));
    }
    ...
    fn npm_version_is_read_from_package_json_without_running_anything() {
        ...
        std::fs::write(
            &p,
            r#"{"name":"@anthropic-ai/claude-code","version":"2.1.267"}"#,
        )
        .unwrap();
