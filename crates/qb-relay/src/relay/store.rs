...
pub fn store_path() -> std::path::PathBuf {
    crate::paths::state_dir().join("relay.json")
}
...
impl RelayStore {
    ...
    pub fn view_of(&self, t: RelayTarget) -> Vec<ProviderView> {
        ...
        let mut list: Vec<&StoredProvider> = self
            .providers
            .iter()
    ...
    pub fn upsert(&mut self, input: ProviderInput) -> Result<String> {
        ...
        meta.base_url = crate::endpoint::normalize_base_url(&meta.base_url);
...
mod tests {
    ...
    fn upsert_normalizes_a_pasted_endpoint() {
        ...
        assert_eq!(
            st.get(&id).unwrap().meta.base_url,
    ...
    fn blank_key_string_also_keeps_the_existing_one() {
        ...
        s.upsert(again).unwrap();
        assert_eq!(
            s.get(&id).unwrap().plain_key().as_deref(),
            Some("sk-keep-me")
        );
    }
    ...
    fn removing_the_active_one_clears_the_pointer() {
        ...
        s.active
            .insert(RelayTarget::Codex.as_str().into(), id.clone());
        assert!(s.remove(&id));
    ...
    fn duplicate_lands_right_below_the_source() {
        let mut s = RelayStore::default();
        let a = s
            .upsert(input("A", RelayTarget::Codex, Some("sk-a")))
            .unwrap();
    ...
    fn targets_are_isolated_from_each_other() {
        ...
        let mut s = RelayStore::default();
        s.upsert(input("codex 的", RelayTarget::Codex, None))
            .unwrap();
