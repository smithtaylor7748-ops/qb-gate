...
pub fn store_path() -> std::path::PathBuf {
    crate::paths::state_dir().join("profiles.json")
}
...
pub fn capture(name: &str) -> Profile {
    Profile {
        ...
        account: crate::accounts::slots()
            ...
            .find(|s| s.active)
            .map(|s| s.label),
        // 不再从旧库抓「当前启用的中转站」：那张表已经迁走，而且档案
        // 本来就不该去写用户真实的 Claude / Codex 配置。见下面 apply() 的说明。
...
pub fn apply(id: &str) -> Result<ApplyReport> {
    ...
    // ---- 账户 ----
    match &p.account {
        Some(label) => match crate::usecase::account_ops::switch(label) {
    ...
    // **档案不再改中转站配置。**
    //
    // `relay_activate` 那个命令早就被停用了，理由是它直接写用户真实的
    // `~/.claude/settings.json` / `~/.codex/config.toml`，跟「中转环境只写自己
    // 那个独立目录」的设计正面冲突。但当时漏了这条路径：命令被掐了，
    // `profile::apply` 还在调同一个 `relay::activate` —— 禁用等于没禁，
    // 从档案这边照样能写进用户的真实配置。
    //
    // 而且老档案里存的 id 指向的是已经迁走的旧库，照着切也切不对。
    // 这里只如实报告，一个字节都不写。
    for t in RelayTarget::ALL {
        match p.relays.get(t.as_str()) {
            Some(_) => skipped.push(format!(
                "{}（档案里的中转站记录来自旧版，请到中转站页面切换）",
                t.label()
            )),
