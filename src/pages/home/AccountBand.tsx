...
import { Button, Card, ConfirmDialog, Pill, useToast, fmtDaysLeft } from '../../ui';

import { needsLogin, requestNewSlot, requestSwitch } from '../accounts/AccountDialogs';
...
export default function AccountBand() {
  ...
  const autoPage = Math.floor(Math.max(0, shown.findIndex((s) => s.active)) / PER_PAGE);
          ...
          <div className="flex min-w-0 flex-col gap-1.5">
            <div className="mb-1 flex items-center gap-2">
              ...
              <span className="ml-auto flex flex-shrink-0 gap-1.5">
                ...
                <Button size="sm" onClick={() => void openUrl(CLAUDE_USAGE_URL)}>
                  Usage
                </Button>
              ...
              <p className="notice">
                {slots.length === 0 ? (
                  ...
                  <>
                    ...
                    没有槽位时，Claude Code 用的是它自己的默认目录 <code>~\.claude</code>。
                  </>
                ) : (
                  ...
                  >
                    ...
                    <span className="slotrow-side">
                      ...
                      {s.active ? (
                        ...
                        <Button size="sm" disabled={!!busy} onClick={() => requestSwitch(s.label)}>
                          {switchLabel(s)}
                        </Button>
                      )}
                    </span>
          ...
          <div className="flex min-w-0 flex-col gap-2 md:border-l md:border-line md:pl-4">
            ...
            <div className="tilegrid">
              ...
              <Tile
                icon={<Wine size={18} />}
                name="酒馆"
                note={running ? '运行中 · 再点只打开页面' : '起桥接与酒馆 · 最长 80 秒'}
                task={tavernTask}
                disabled={!!busy}
