...
const FONTS_SC = [
  ...
  'KaiTi', 'FangSong', 'DengXian', 'PingFang SC', 'Hiragino Sans GB',
  'STHeiti', 'STSong', 'Songti SC', 'Source Han Sans CN', 'Source Han Sans SC',
...
const FONTS_CN_VENDOR = [
  'MiSans', 'MIUI', 'HarmonyOS Sans SC', 'HarmonyOS Sans', 'HONOR Sans',
  'OPPO Sans', 'vivo Sans', 'Alibaba PuHuiTi', 'Alibaba Sans',
  'DingTalk JinBuTi', 'Douyin Sans', 'HYQiHei',
...
function detectTimezoneOffset(): DetectOutcome {
  ...
  return { raw: `UTC${sign}${Math.abs(utcHours)}`, score: offset === -480 ? 0.7 : 0 };
}
...
function detectFonts(): DetectOutcome {
  ...
  const hit = [...sc, ...tc];
  const raw = hit.length ? hit.slice(0, 4).join(', ') + (hit.length > 4 ? '…' : '') : '未检测到';
  return { raw, score };
}
...
function detectVendorFonts(): DetectOutcome {
  ...
  const raw = hit.length ? hit.slice(0, 3).join(', ') + (hit.length > 3 ? '…' : '') : '未检测到';
  return { raw, score };
}
  ...
  return new Promise((resolve) => {
    ...
    const finish = () => {
      ...
      if (ips.length === 0) {
        ...
        resolve({ raw: `候选地址泄露（${ips.slice(0, 2).join(', ')}）`, score: 0.5 });
      }
    };
