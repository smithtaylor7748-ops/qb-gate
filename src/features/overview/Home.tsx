...
export default function Home() {
  ...
  // `Record<string, StepRecord>` 在 ts-rs 生成的类型里是 `{[k in string]?: …}`，
  // 也就是取出来可能是 undefined —— 手写的那份类型声明成了必有值，
  // 于是这里一直在读一个「编译器以为一定存在」的东西。
  const skipped = Object.values(progress.steps).filter(
    (s) => s?.state === "skipped",
  ).length;
   ...
   * 面板不替他判定，只把两个自测接口刷新一遍。所以体检完那一格可能仍然是
   * 「未检测」，这是对的。
   */
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            >
              {checks.busy ? checks.phase : "一键全面体检"}
            </Button>
          </>
        }
      />

      {/* 四格里点任意一格开小窗，就地做完，不跳页。小窗本体挂在 Shell 上。 */}
      <ScoreBand />
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <Button onClick={() => openSecurity("purity")}>重新复核</Button>
              </div>
            </div>
          </div>
        </Card>
