...
![中转站](docs/screenshots/relay.png)

### 主模型与小模型分开配

Claude Code 会拿一个**小模型**跑后台轻活（补会话标题之类）。很多中转站两档
不是同一个名字，只配主模型的话那些后台请求会报一个**跟你正在做的事完全无关**
的错，极难联想。所以每条供应商都能单独配小模型，落到
`ANTHROPIC_SMALL_FAST_MODEL`；留空就不设这个变量，交给 Claude Code 用默认值。

粘地址也不用讲究：`https://x.com/v1/messages` 这种完整端点会自动归一成
`https://x.com/v1` —— 归一在保存时做，手填、预设、从当前配置导入三条路都过。
...
本机侧那四项之外，v0.11.0 又加了两项：

| 新增 | 查什么 |
|---|---|
| **环境变量残留** | `HKCU\Environment` 与进程环境里的 `ANTHROPIC_BASE_URL` / `*_API_KEY` / `HTTPS_PROXY` / `CLAUDE_CONFIG_DIR`。一个忘了删的 `ANTHROPIC_BASE_URL` 会让 Claude Code **悄悄走一个死掉的中转站**，而中转站页上显示的是「官方直连」—— 两处说的不是一件事 |
| **出口一致性** | 面板绕过系统代理测一次、跟随系统代理再测一次。两边国家不一样就说明**有程序会从另一个国家出去** —— 专治「面板说我在美国，为什么还是被当成国内」 |

> 环境变量的值也是掩码过的：Key 类只说「已设置」，地址类**只留 host** ——
> 有些中转站把 token 放在路径里，显示全量等于把它印在截图上。
>
> 出口一致性的结果**不进门禁判定**。门禁的输入永远只来自绕过代理的那一份，
> 否则随便一个本地代理就能把出口伪装成白名单里那个。
